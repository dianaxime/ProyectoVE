...
const createAdmin = async (req, res) => {
    ...
    if (isEmpty(email) || isEmpty(first_name) || isEmpty(last_name) || isEmpty(password)) {
        errorMessage.error = 'Email, password, first name and last name cannot be empty';
        return res.status(status.bad).send(errorMessage);
    }
    ...
    if (!validatePassword(password)) {
        errorMessage.error = 'Password must be more then eight(8) characters';
        return res.status(status.bad).send(errorMessage);
    }

    const hashedPassword = hashPassword(password);

    const createUserQuery = `INSERT INTO users(email, first_name, last_name, password, is_admin, created_on)
    VALUES ($1, $2, $3, $4, $5, $6)
    returning *`;
